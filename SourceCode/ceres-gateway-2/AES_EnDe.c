...
#ifdef _MSC_VER
#include <Windows.h>
#define __msecSleep(ms) Sleep(ms)
#endif

#ifdef LINUX
#include <pthread.h>
#define __msecSleep(ms) usleep(ms * 1000)
#endif
...
#define g_in    g->in
#define g_out   g->out
#define g_state g->state

// The array that stores the round keys.
#define g_RoundKey g->RoundKey
...
unsigned char getRconValue(int num)
{
    return Rcon[num];
}
...
void KeyExpansion(struct global *g)
{
    int i, n, x, y, z;
    ...
    {
        x = (i - 1) * 4;
        temp[0] = g_RoundKey[x + 0];
        temp[1] = g_RoundKey[x + 1];
        temp[2] = g_RoundKey[x + 2];
        temp[3] = g_RoundKey[x + 3];
        ...
        if (i % g_Nk == 0)
        {
            ...
            temp[0] =  temp[0] ^ getRconValue(i / g_Nk);
        ...
        y = i * 4;
        z = (i - g_Nk) * 4;
        g_RoundKey[y + 0] = g_RoundKey[z + 0] ^ temp[0];
        g_RoundKey[y + 1] = g_RoundKey[z + 1] ^ temp[1];
        g_RoundKey[y + 2] = g_RoundKey[z + 2] ^ temp[2];
        g_RoundKey[y + 3] = g_RoundKey[z + 3] ^ temp[3];
    }
}
...
void AddRoundKey(struct global *g, int round)
{
    int i, j, x, y;
...
void SubBytes(struct global *g)
{
    ...
    {
        for(j = 0; j < 4; j++)
            g_state[i][j] = getSBoxValue(g_state[i][j]);
    }
}
...
void ShiftRows(struct global *g)
{
    unsigned char temp;
...
void MixColumns(struct global *g)
{
    int i;
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        Tm = g_state[0][i] ^ g_state[1][i] ;
        Tm = xtime(Tm);
        g_state[0][i] ^= Tm ^ Tmp ;
        Tm = g_state[1][i] ^ g_state[2][i] ;
        Tm = xtime(Tm);
        g_state[1][i] ^= Tm ^ Tmp ;
        Tm = g_state[2][i] ^ g_state[3][i] ;
        Tm = xtime(Tm);
        g_state[2][i] ^= Tm ^ Tmp ;
        Tm = g_state[3][i] ^ t ;
        Tm = xtime(Tm);
...
unsigned char getSBoxInvert(int num)
{
    return rsbox[num];
}
...
// The SubBytes Function Substitutes the values g_in the
// g_state matrix with values g_in an S-box.
void InvSubBytes(struct global *g)
{
    int i, j;
    ...
    {
        for(j = 0; j < 4; j++)
            g_state[i][j] = getSBoxInvert(g_state[i][j]);
    }
}
...
// Each row is shifted with different offset.
// Offset = Row number. So the first row is not shifted.
void InvShiftRows(struct global *g)
{
    unsigned char temp;

    // Rotate first row 1 columns to right
    temp = g_state[1][0];
    g_state[1][0] = g_state[1][3];
...
void InvMixColumns(struct global *g)
{
    int i;
...
void InvCipher(struct global *g)
{
    int i, j, x, round = 0;
...
int AES_Encrypt(int nNumRounds,
                unsigned char *in_PlainText,
                int nSize_in,
                unsigned char *sKey,
                int nSize_skey,
                unsigned char *out_Encry)
{
    struct global global;
    struct global *g = &global;
    ...
    default:
        return AES_ER_WRONG_NUM_ROUNDS;
    }

    if(!sKey || !in_PlainText || !out_Encry ||
            nSize_in <= 0 || nSize_skey <= 0)
        return AES_ER_INVALID_PARAMETER;

    memset(g, 0x0, sizeof(*g));

    // Calculate g_Nk and g_Nr from the recieved value.
    g_Nk = nNumRounds >> 5;   //4,6,8
    g_Nr = g_Nk + 6;          //10,12,14

    if(nSize_in > 32)
        nSize_in = 32;

    memcpy(g_in, in_PlainText, nSize_in);

    if(nSize_skey > 32)
        nSize_skey = 32;

    memcpy(g_Key, sKey, nSize_skey);
...
int AES_Decrypt(int nNumRounds,
                unsigned char *in_EncryText,
                int nSize_in,
                unsigned char *sKey,
                int nSize_skey,
                unsigned char *out_Plain)
{
    ...
    if(!sKey || !in_EncryText || !out_Plain ||
            nSize_in <= 0 || nSize_skey <= 0)
        return AES_ER_INVALID_PARAMETER;
    ...
    memcpy(g_in, in_EncryText, nSize_in);
